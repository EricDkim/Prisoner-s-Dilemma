...
function DoCustomValidation()
{
  ...
  {
    sfm_show_error_msg('The Password and Re-typed password do not match!',frm.pass1);
    return false;
  }
  else
  {  
...
function TestRequiredInput(objValue, strError)
{
    ...
    if (!ret)
    {
        if (!strError || strError.length == 0)
        {
            strError = objValue.name + " Field cannot be left empty or blank";
        } //if 
        sfm_show_error_msg(strError, objValue);
    }
    return ret;
...
function validateInput(strValidateStr, objValue, strError)
{
    ...
    {
	/*case "mismatch":
	case "confirmequality":
		{
			ret = ConfirmMisMatchPasswords(objValue, strError)
			break;
		}*/
