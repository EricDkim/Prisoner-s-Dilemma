<?php
    ...
    function show_game($id){
    	...
    	$tr = $ti-$t2;
    	if($tr <= 0){
    	    $sql = "UPDATE games_iterative SET status=6 WHERE id='$id'";
    	    $dbc->query($sql);
            if($cpd == 0){              
        ...
        elseif( ($opd!=0) AND ($cpd!=0) ){
            $sql = "SELECT status FROM games_iterative WHERE id='$id'";
            $query = $dbc->query($sql);
            $fetch = $query->fetch_assoc();
            $st = $fetch['status']+1;
            if($st != 6){
...
TIMESTAMPDIFF(MINUTE, login_history.log_in , NOW()) < 60 and login_history.log_out = '0000-00-00 00:00:00' AND login_history.busy = 0 AND login_history.isAdmin = 0 ORDER BY rand() LIMIT 1";
    ...
    elseif($busy){
    	/* Get game id */
    	$sql = "SELECT * FROM games_iterative WHERE (player1='$login_id' OR player2='$login_id') AND (status!=6)";
    	$query = $dbc->query($sql);
    	$fetch = $query->fetch_assoc();
    	$id = $fetch['id'];
    	/* show game */
...
$('.btn_unbusy').click(function(){
    ...
    <?php
    $sql = "SELECT * FROM games_iterative WHERE (player1='$login_id' OR player2='$login_id') AND (status=6) ORDER BY id DESC LIMIT 1";
    $query = $dbc->query($sql);
    $fetch = $query->fetch_assoc();
    $p1 = $fetch['player1'];
    $op = ($p1 != $login_id) ? $p1 : $fetch['player2'];
